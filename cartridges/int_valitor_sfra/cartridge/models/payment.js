...
function applicablePaymentMethods(paymentMethods) {
	return collections.map(paymentMethods, function (method) {
		return {
			ID: method.ID,
			name: method.name,
			imageURL: method.image ? method.image.getURL().toString() : null
		};
	});
}

/**
 * Returns selected payment method or the first applicable paymentmethod
 * @param {dw.util.ArrayList<dw.order.PaymentMethod>} paymentMethods - An ArrayList of
 *      applicable payment methods that the user could use for the current basket.
 * @returns {dw.order.PaymentMethod} selected paymentmethod
 */
function getSelectedPaymentMethod(paymentMethods) {

	if (!paymentMethods) return null;

	var paymentMethodID = session.custom.selectedPaymentMethod ? session.custom.selectedPaymentMethod : '';
...
function applicablePaymentCards(paymentCards) {
	return collections.map(paymentCards, function (card) {
		return {
			...
			name: card.name,
			imageURL: card.image ? card.image.getURL().toString() : null
		};
	});
}
...
function getSelectedPaymentInstruments(selectedPaymentInstruments) {
	return collections.map(selectedPaymentInstruments, function (paymentInstrument) {
		...
		} else if (paymentInstrument.paymentMethod.indexOf('VALITOR') != -1) {
			results.paymentMethodName = PaymentMgr.getPaymentMethod(paymentInstrument.paymentMethod).name;
		}
		return results;
	});
}
...
function Payment(currentBasket, currentCustomer, countryCode) {
	...
	this.selectedPaymentMethod = getSelectedPaymentMethod(paymentMethods);    
